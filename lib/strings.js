...
(function (root, factory) {
  ...
  const INSTRUMENTS = {
    ...
    doublebass: {
      ...
      family: "strings",
      style: "fingerboard",
      // Vertical standing-bass chart (Simandl now; Rabbath can plug into `methods`)
      chart: "upright",
      neckFrets: 19,
      methods: ["simandl"],
      defaultMethod: "simandl",
      strings: 4,
      openMidi: [28, 33, 38, 43], // E1 A1 D2 G2
      stringNames: ["E", "A", "D", "G"],
      presets: {
        ...
        solo: {
          ...
          label: "Solo",
          detail: "F#BEA",
          openMidi: [30, 35, 40, 45], // F#1 B1 E2 A2
        },
      },
    },
  ...
  /**
   * Upright-bass methods overlay named positions on a vertical fingerboard.
   *
   * Each method is { id, label, fingering, positions[] }.
   * A position is defined in semitones above the open string (same on every
   * string in fourths tuning) via `fingers`: finger number → fret.
   *
   * Simandl uses closed 1–2–4 (index, middle, pinky) spanning a whole tone.
   * Rabbath (6 positions, 1–2–3–4) can be added as another entry with the
   * same shape — the upright renderer does not hard-code Simandl.
   */
  function simandlPosition(id, label, firstFinger, color) {
    return {
      id,
      label,
      firstFinger,
  ...
  const BASS_METHODS = {
    simandl: {
      id: "simandl",
      label: "Simandl",
      detail: "1–2–4 fingering · ½ through VII on the neck; thumb from the octave",
      fingering: "1-2-4",
      // Neck = Simandl Book 1 (nut through the octave harmonic).
      // Thumb = octave harmonic and above (Book 2 register). Shared fret 12 is the 8ve.
      regions: [
        { id: "neck", label: "Neck", detail: "½–VII · 1–2–4", startFret: 0, endFret: 12 },
        { id: "thumb", label: "Thumb", detail: "octave harmonic and above", startFret: 12, endFret: 19 },
      ],
      positions: [
        simandlPosition("half", "½", 1, "#67e8f9"),
        simandlPosition("I", "I", 2, "#3db8ff"),
        simandlPosition("II", "II", 3, "#818cf8"),
        simandlPosition("IIhalf", "II½", 4, "#c084fc"),
        simandlPosition("III", "III", 5, "#f472b6"),
        simandlPosition("IIIhalf", "III½", 6, "#ff4d6d"),
        simandlPosition("IV", "IV", 7, "#fb923c"),
        simandlPosition("V", "V", 8, "#f0c040"),
        simandlPosition("Vhalf", "V½", 9, "#a3e635"),
        simandlPosition("VI", "VI", 10, "#3ecf8e"),
        simandlPosition("VIhalf", "VI½", 11, "#2dd4bf"),
        simandlPosition("VII", "VII", 12, "#38bdf8"),
      ],
    },
  };
  ...
  /**
   * Normalize disabled-string mask (low → high). Accepts boolean[] or index[].
   * Disabled strings are forced mute in chord/scale charts.
   */
  function normalizeDisabledStrings(disabled, stringCount) {
    const n = Math.max(0, Number(stringCount) || 0);
    const out = Array.from({ length: n }, () => false);
    if (disabled == null || n === 0) return out;
    if (!Array.isArray(disabled)) return out;
    if (disabled.length === n && disabled.every((d) => typeof d === "boolean")) {
      return disabled.map((d) => Boolean(d));
    }
    disabled.forEach((item) => {
      const i = Number(item);
      if (Number.isInteger(i) && i >= 0 && i < n) out[i] = true;
    });
    return out;
  }

  function hasDisabledStrings(disabled) {
    return Array.isArray(disabled) && disabled.some(Boolean);
  }

  function enabledStringCount(disabled) {
    if (!Array.isArray(disabled) || !disabled.length) return 0;
    return disabled.reduce((n, d) => n + (d ? 0 : 1), 0);
  }

  /** Guitar-oriented active-string sets (indices low → high). */
  const GUITAR_STRING_SETS = {
    all: {
      id: "all",
      label: "All strings",
      detail: "EADGBE",
    ...
    treble3: {
      id: "treble3",
      label: "Treble 3",
      detail: "G·B·E — triads on top three",
      disabled: [true, true, true, false, false, false],
    },
    treble4: {
      id: "treble4",
      label: "Treble 4",
      detail: "D·G·B·E — add 7ths on four strings",
      disabled: [true, true, false, false, false, false],
    },
  };

  function listStringSets(instrumentId) {
    if (instrumentId === "guitar6") return Object.values(GUITAR_STRING_SETS);
    const inst = INSTRUMENTS[instrumentId];
    if (!inst) return [];
    return [
      {
        id: "all",
        label: "All strings",
        detail: `${inst.strings} open`,
        disabled: Array.from({ length: inst.strings }, () => false),
      },
    ];
  }

  function applyStringSet(instrumentId, setId) {
    const sets = listStringSets(instrumentId);
    const found = sets.find((s) => s.id === setId) || sets[0];
    return found ? found.disabled.slice() : null;
  }

  function matchStringSetId(instrumentId, disabled) {
    const inst = INSTRUMENTS[instrumentId];
    if (!inst) return null;
    const mask = normalizeDisabledStrings(disabled, inst.strings);
    const sets = listStringSets(instrumentId);
    const hit = sets.find((s) => s.disabled.length === mask.length && s.disabled.every((d, i) => d === mask[i]));
    return hit ? hit.id : null;
  }

  function scoreShape(frets, openMidi, capo, rootPc, tonePcs, opts = {}) {
    const disabled = normalizeDisabledStrings(opts.disabledStrings, frets.length);
    ...
    // Only penalize mutes on strings the player still wants to use
    const muted = frets.filter((f, i) => f == null && !disabled[i]).length;
  ...
  function searchVoicing(tuning, capo, rootPc, intervals, opts = {}) {
    ...
    const disabled = normalizeDisabledStrings(opts.disabledStrings, tuning.length);
    ...
    function dfs(i, frets) {
      if (i === n) {
        const sc = scoreShape(frets, opens, capo, rootPc, toneSet, { disabledStrings: disabled });
        if (sc < bestScore) {
          bestScore = sc;
          best = frets.slice();
        }
  ...
  function applyDisabledToFrets(frets, disabled) {
    if (!hasDisabledStrings(disabled)) return frets;
    return frets.map((f, i) => (disabled[i] ? null : f));
  }

  function defaultSearchMaxFret(inst, disabled) {
    const enabled = enabledStringCount(disabled);
    if (inst.style === "fingerboard") return 7;
    // Fewer active strings → allow a slightly wider box so triads/7ths still fit
    if (enabled > 0 && enabled <= 3) return 7;
    if (enabled === 4) return 6;
    return 5;
  }

  /**
   * @param {object} [opts]
   * @param {boolean[]|number[]} [opts.disabledStrings] low→high mask or indices to mute
   * @returns {{ frets: (number|null)[], fingers: (number|null)[], baseFret: number, barre: number|null, midis: number[], missing: string[], disabledStrings: boolean[] }}
   */
  function resolveChordShape(instrumentId, root, intervals, capo = 0, tuning = null, opts = {}) {
    const inst = INSTRUMENTS[instrumentId];
    if (!inst) throw new Error(`Unknown instrument: ${instrumentId}`);
    const c = clampCapo(capo);
    const openMidi = normalizeTuning(tuning || inst.openMidi, instrumentId);
    const disabled = normalizeDisabledStrings(opts.disabledStrings, openMidi.length);
    const rootPc = noteToPc(root);
    const tonePcs = new Set(intervals.map((iv) => (rootPc + ((iv % 12) + 12) % 12) % 12));
    const searchOpts = {
      disabledStrings: disabled,
      maxFret: defaultSearchMaxFret(inst, disabled),
    };

    let frets = null;
    let fingers = null;
    // Curated full-neck shapes assume all strings; skip when any string is disabled
    if (inst.voicing === "open") {
      frets = applyDisabledToFrets(openStringVoicing(openMidi, c, tonePcs), disabled);
    } else if (
      !hasDisabledStrings(disabled) &&
      instrumentId === "guitar6" &&
      isStandardTuning(instrumentId, openMidi)
    ) {
    ...
    if (!frets) {
      frets = searchVoicing(openMidi, c, rootPc, intervals, searchOpts);
    }
    if (!frets) {
      frets = openMidi.map(() => null);
    } else {
      frets = applyDisabledToFrets(frets, disabled);
    }

    if (inst.voicing !== "open" && !coversTones(frets, openMidi, c, tonePcs)) {
      const wider = searchVoicing(openMidi, c, rootPc, intervals, {
        ...searchOpts,
        maxFret: Math.max(searchOpts.maxFret, 8),
      });
      if (
        wider &&
        scoreShape(wider, openMidi, c, rootPc, tonePcs, { disabledStrings: disabled }) <
          scoreShape(frets, openMidi, c, rootPc, tonePcs, { disabledStrings: disabled })
      ) {
        frets = applyDisabledToFrets(wider, disabled);
        fingers = null;
      }
    }
    ...
    return {
      ...
      disabledStrings: disabled.slice(),
    };
  }
   ...
   * @returns {{ dots: { string: number, fret: number, midi: number, note: string, isRoot: boolean }[], baseFret: number, fretsShown: number }}
   */
  function resolveScaleDiagram(instrumentId, root, intervals, capo = 0, tuning = null, opts = {}) {
    ...
    openMidi.forEach((open, stringIndex) => {
      if (disabled[stringIndex]) return;
      const base = open + c;
  ...
  function isUprightChart(instrumentId) {
    return getInstrument(instrumentId)?.chart === "upright";
  }

  function getBassMethod(methodId) {
    return BASS_METHODS[methodId] || null;
  }

  function listBassMethods(instrumentId) {
    const inst = getInstrument(instrumentId);
    const ids = inst?.methods || [];
    return ids.map((id) => BASS_METHODS[id]).filter(Boolean);
  }

  function defaultBassMethodId(instrumentId) {
    const inst = getInstrument(instrumentId);
    if (!inst) return null;
    if (inst.defaultMethod && BASS_METHODS[inst.defaultMethod]) return inst.defaultMethod;
    const listed = listBassMethods(instrumentId);
    return listed[0]?.id || null;
  }

  function defaultBassPositionIds(methodId) {
    const method = getBassMethod(methodId);
    return method ? method.positions.map((p) => p.id) : [];
  }

  function normalizeBassPositionIds(methodId, ids) {
    const valid = new Set(defaultBassPositionIds(methodId));
    if (!Array.isArray(ids)) return defaultBassPositionIds(methodId);
    return ids.filter((id) => valid.has(id));
  }

  function normalizeBassView(view) {
    return view === "focus" ? "focus" : "full";
  }

  function normalizeBassBoardLayout(layout) {
    return layout === "one" ? "one" : "split";
  }

  function methodRegions(method, neckFrets) {
    const endCap = Math.max(1, Math.round(neckFrets) || 12);
    const listed = Array.isArray(method?.regions) ? method.regions : [];
    const out = [];
    listed.forEach((r) => {
      if (!r || r.id == null) return;
      const start = Math.max(0, Math.round(Number(r.startFret) || 0));
      const end = Math.min(endCap, Math.round(Number(r.endFret)));
    ...
    if (!out.length) {
      out.push({ id: "fingerboard", label: "Fingerboard", detail: "", startFret: 0, endFret: endCap });
    }
    return out;
  }

  function clipFretRange(a, b) {
    if (!a || !b) return null;
    const start = Math.max(Number(a.startFret), Number(b.startFret));
    const end = Math.min(Number(a.endFret), Number(b.endFret));
    if (!Number.isFinite(start) || !Number.isFinite(end) || end < start) return null;
    return { startFret: start, endFret: end };
  }

  /** Distance from the nut as a fraction of vibrating string length (12-TET). */
  ...
  /** Width of one fret cell as a fraction of scale length. */
  function fretCellRatio(fret) {
    const n = Math.round(Number(fret));
    if (!Number.isFinite(n) || n < 1) return 0;
    return fretDistanceFromNut(n) - fretDistanceFromNut(n - 1);
  }

  function fretCellRatios(fromFret, toFret) {
    const start = Math.max(1, Math.round(fromFret));
    const end = Math.max(start, Math.round(toFret));
    const ratios = [];
    for (let f = start; f <= end; f++) ratios.push(fretCellRatio(f));
    return ratios;
  }

  function positionFretRange(pos) {
    const frets = Object.values(pos.fingers).map(Number).filter((n) => Number.isFinite(n));
    if (!frets.length) return { min: 0, max: 0 };
    return { min: Math.min(...frets), max: Math.max(...frets) };
  }

  function positionsCoveringFret(method, fret, activeIds) {
    if (!method) return [];
    const filter = Array.isArray(activeIds) && activeIds.length ? new Set(activeIds) : null;
    return method.positions.filter((pos) => {
      if (filter && !filter.has(pos.id)) return false;
      const { min, max } = positionFretRange(pos);
      return fret >= min && fret <= max;
    });
  }

  function fingersAtFret(method, fret, activeIds) {
    const hits = [];
    positionsCoveringFret(method, fret, activeIds).forEach((pos) => {
      Object.entries(pos.fingers).forEach(([finger, at]) => {
        if (Number(at) === Number(fret)) {
          hits.push({
            positionId: pos.id,
  ...
  /** Prefer 1st finger when a note belongs to several overlapping positions. */
  function preferredFinger(hits) {
    if (!hits?.length) return null;
    const asFirst = hits.find((h) => h.finger === 1);
    if (asFirst) return asFirst.finger;
    const ranked = hits.slice().sort((a, b) => a.finger - b.finger);
    return ranked[0].finger;
  }

  /**
   * Visible nut→bridge crop for the enabled positions.
   * No positions selected → full neck (open through neckFrets).
   */
  function neckRangeForPositions(method, activeIds, maxFret) {
    const endCap = Math.max(1, Math.round(maxFret) || 12);
    const ids = Array.isArray(activeIds) ? activeIds : [];
    if (!method || !ids.length) return { startFret: 0, endFret: endCap };
    let min = Infinity;
    let max = -Infinity;
    method.positions.forEach((pos) => {
      if (!ids.includes(pos.id)) return;
      const range = positionFretRange(pos);
      min = Math.min(min, range.min);
      max = Math.max(max, range.max);
    });
    if (!Number.isFinite(min) || !Number.isFinite(max)) return { startFret: 0, endFret: endCap };
    const startFret = min <= 2 ? 0 : Math.max(0, min - 1);
    const endFret = Math.min(endCap, Math.max(startFret + 1, max));
    return { startFret, endFret };
  }

  function uniquePcNames(midis) {
    const seen = new Set();
    const names = [];
    (midis || []).forEach((midi) => {
      const pc = midiPitchClass(midi);
  ...
  function uniqueSortedMidis(midis) {
    return [...new Set((midis || []).filter((n) => Number.isFinite(n)))].sort((a, b) => a - b);
  }

  function lowestPerPitchClass(midis) {
    const best = new Map();
    uniqueSortedMidis(midis).forEach((midi) => {
      const pc = midiPitchClass(midi);
      if (!best.has(pc)) best.set(pc, midi);
    });
    return [...best.values()].sort((a, b) => a - b);
  }

  /**
   * Vertical upright-bass chart: all matching tones on the neck, Simandl (or
   * other method) position bands, and 12-TET fret spacing toward the bridge.
   * Used for both chords and scales so progressions stay on the same map.
   */
  function resolveUprightDiagram(instrumentId, root, intervals, capo = 0, tuning = null, opts = {}) {
    const inst = INSTRUMENTS[instrumentId];
    if (!inst) throw new Error(`Unknown instrument: ${instrumentId}`);
    const c = clampCapo(capo);
    const openMidi = normalizeTuning(tuning || inst.openMidi, instrumentId);
    ...
    const rootPc = noteToPc(root);
    const tonePcs = new Set(intervals.map((iv) => (rootPc + ((iv % 12) + 12) % 12) % 12));
    const neckFrets = Math.max(1, opts.neckFrets || inst.neckFrets || 12);
    const methodId = opts.methodId || defaultBassMethodId(instrumentId);
    const method = getBassMethod(methodId);
    const activePositions = method
      ? normalizeBassPositionIds(methodId, opts.activePositions)
      : [];
    const filterPositions = activePositions.length > 0;
    const view = normalizeBassView(opts.view);
    const focused = view === "focus" ? neckRangeForPositions(method, activePositions, neckFrets) : null;
    const display = focused || { startFret: 0, endFret: neckFrets };
    const regions = methodRegions(method, neckFrets);
    ...
    openMidi.forEach((open, stringIndex) => {
      ...
      for (let f = 0; f <= neckFrets; f++) {
        ...
        if (!tonePcs.has(pc)) continue;
        const hits =
          f > 0 && method && filterPositions ? fingersAtFret(method, f, activePositions) : [];
        const inPosition = f === 0 || !filterPositions || hits.length > 0;
        dots.push({
          string: stringIndex,
          fret: f,
          midi,
          ...
          isRoot: pc === rootPc,
          inPosition,
          finger: preferredFinger(hits),
          positions: hits,
        });
      }
    });

    const emphasized = dots.filter((d) => d.inPosition);
    const playDots = emphasized.length ? emphasized : dots;
    const allMidis = uniqueSortedMidis(playDots.map((d) => d.midi));
    const midis = opts.isScale ? allMidis : lowestPerPitchClass(allMidis);
    const have = new Set(midis.map(midiPitchClass));
    const missing = [...tonePcs].filter((pc) => !have.has(pc)).map((pc) => NOTES[pc]);

    const positionBands = (method ? method.positions : []).map((pos) => {
      const range = positionFretRange(pos);
      return {
        id: pos.id,
        label: pos.label,
        color: pos.color,
        firstFinger: pos.firstFinger,
        minFret: range.min,
        maxFret: range.max,
        enabled: activePositions.includes(pos.id),
      };
    });
    ...
    return {
      chart: "upright",
      dots,
      positions: positionBands,
      view,
      startFret: display.startFret,
      endFret: display.endFret,
      neckFrets,
      regions,
      fretsShown: Math.max(1, display.endFret - (display.startFret > 0 ? display.startFret : 0)),
      fretRatios: fretCellRatios(Math.max(1, display.startFret || 1), display.endFret),
      stringOrder: "high-left",
      methodId: method?.id || null,
      methodLabel: method?.label || null,
      fingering: method?.fingering || null,
      activePositions: activePositions.slice(),
      midis,
      pcNames: uniquePcNames(midis),
      missing,
      capo: c,
      instrumentId,
      openMidi: openMidi.slice(),
  ...
  return {
    ...
    listBassMethods,
    defaultBassMethodId,
    defaultBassPositionIds,
    normalizeBassPositionIds,
    normalizeBassView,
    normalizeBassBoardLayout,
    methodRegions,
    clipFretRange,
    fretDistanceFromNut,
    fretCellRatio,
    fretCellRatios,
    positionsCoveringFret,
    fingersAtFret,
    preferredFinger,
    neckRangeForPositions,
    resolveUprightDiagram,
    intervalsKey,
    midiPitchClass,
    noteToPc,
  };

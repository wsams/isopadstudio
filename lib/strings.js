...
(function (root, factory) {
  ...
  /**
   * Scale box: for each string, frets (relative to capo) in a window that hit scale tones.
  ...
  function resolveScaleDiagram(instrumentId, root, intervals, capo = 0, tuning = null, opts = {}) {
    ...
    const rootPc = noteToPc(root);
    const tonePcs = new Set(intervals.map((iv) => (rootPc + ((iv % 12) + 12) % 12) % 12));
    const window = opts.window ?? (inst.style === "fingerboard" ? 7 : 5);
    const start = opts.startFret ?? 0; // relative to capo
    ...
    const pressed = dots.map((d) => d.fret).filter((f) => f > 0);
    let baseFret = 1;
    let displayDots = dots;
    if (pressed.length && Math.max(...pressed) > 4 && start === 0) {
      const min = Math.min(...pressed.filter((f) => f > 0));
      if (min > 1) {
        baseFret = min;
        displayDots = dots.map((d) => ({
          ...d,
          fret: d.fret === 0 ? 0 : d.fret - baseFret + 1,
          absoluteFret: d.fret,
        }));
      }
    }
